...
{ Appenders -------------------------------------------------------------------}
  ...
  ILogRollingFileAppender = interface(ILogAppender)
    ...
    procedure RollOver;
    procedure renameLogfile(aName: string);
  end;
...
var
  LogList: TStringList;
begin
  if TempLogFileName > '' then
  begin
    LogList := TStringList.Create;
    if FileExists(TempLogFileName) then
      LogList.LoadFromFile(TempLogFilename);

    LogList.Add(FormatDateTime('dd.mm.yy hh:nn:ss:zzz', now) + ' Start Log4.Shutdown');
    LogList.SaveToFile(TempLogFileName);
    Shutdown; // @tb 14.07.15 Nach dem Shutdown werden die Log-Dateien freigegeben.
    LogList.Add(FormatDateTime('dd.mm.yy hh:nn:ss:zzz', now) + ' Ende Log4.Shutdown - Logdatei sollte freigegeben sein.');
    LogList.SaveToFile(TempLogFileName);
    FreeAndNil(LogList);
  end
  else
...
procedure TLogFileAppender.setFilename(const Value: TFileName);
begin
  SetLogFile(Value);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      showMessage('Log-Datei ' + FFilename + ' konnte nicht angelegt werden');            
    end;
...
procedure TLogRollingFileAppender.renameLogfile(aName: string);
begin
  SetLogFile(aName);
end;
