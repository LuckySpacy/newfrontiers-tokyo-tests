...
uses
  ...
  Classes, System.SysUtils, IBX.IBDatabase, IBX.IBQuery, IBX.IBTable, Graphics, DB,
  o_multivalues, o_bitmuster, o_zahlart, Vcl.StdCtrls,
  o_logging, Lockprovider, Windows, System.Win.Registry, c_types;
...
//    DruckablaufHelper: TDruckablaufHelper;
  ...
  public
   ...
   // procedure setDruckablaufHelper(aWert: TDruckablaufHelper);
    ...
    procedure setServiceVersandart(aVersandart: integer);
...
var
  logFileName: string;
  StammdatenX: TStammdaten;

implementation

uses f_optima, o_mitarbeiter,  Math, Dialogs,
  ShlObj, o_nf, DateUtils, o_optimaStandard, o_geschaeftsbereich, o_voarten,
  rvHunSpell, o_GlobalSpell;
...
{
class function Stammdaten: TStammdaten;
begin
  Result := instance;
end;
}
...
begin
  ...
  if RemotePfad = '' then
    exit;
  if (Tnf.GetInstance.System.DirExist(StammdatenX.RemotePfad + 'Aspell') < 0)  then
    exit;
  if getMitarbeiter = nil then
     exit;
...
begin
  ...
  if (Tnf.GetInstance.System.FileExist(LokalHunspellPfad + 'de.dic') >= 0) then
    System.SysUtils.DeleteFile(LokalHunspellPfad + 'de.dic');
  if (Tnf.GetInstance.System.FileExist(LokalHunspellPfad + 'de.aff') >= 0) then
    System.SysUtils.DeleteFile(LokalHunspellPfad + 'de.aff');
  ...
  GlobalSpell.DllDir  := StammdatenX.LokalHunspellPfad;
  GlobalSpell.DictDir := StammdatenX.LokalHunspellPfad;
  GlobalSpell.CustomDictionaryFilename := StammdatenX.LokalHunspellPfad + 'Custom.woe';
